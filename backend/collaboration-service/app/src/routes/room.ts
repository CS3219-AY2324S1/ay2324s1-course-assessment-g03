...
import { createOneRoom, findUserInRoom, getOneRoomInfo, leaveOneRoom } from "../models/rooms.model";
...
roomRouter.delete("/:roomId/user/:userId", (req: Request, res: Response) => {
  try {
    const { roomId, userId } = roomSchemas.deleteOneRoomByUserIdSchema.parse(req).params
