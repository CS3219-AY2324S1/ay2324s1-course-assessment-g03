...
class Cluster extends pulumi.ComponentResource {
    ...
    createAutoScalingGroup(name, args = {}, opts = {}) {
        ...
        const group = new autoscaling.AutoScalingGroup(name, args, Object.assign({ parent: this }, opts));
    ...
    static createDefaultSecurityGroupEgressRules(name, securityGroup) {
        return [ec2.SecurityGroupRule.egress(`${name}-egress`, securityGroup, new ec2.AnyIPv4Location(), new ec2.AllTraffic(), "allow output to any ipv4 address using any protocol")];
    }
    static createDefaultSecurityGroupIngressRules(name, securityGroup) {
        return [ec2.SecurityGroupRule.ingress(`${name}-ssh`, securityGroup, new ec2.AnyIPv4Location(), new ec2.TcpPorts(22), "allow ssh in from any ipv4 address"),
            // Expose ephemeral container ports to Internet.
            // TODO: Limit to load balancer(s).
            ec2.SecurityGroupRule.ingress(`${name}-containers`, securityGroup, new ec2.AnyIPv4Location(), new ec2.AllTcpPorts(), "allow incoming tcp on any port from any ipv4 address")];
    }
}
